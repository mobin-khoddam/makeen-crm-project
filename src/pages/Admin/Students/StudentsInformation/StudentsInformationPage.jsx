...
import FinancialAffairs
    from "../../../../Container/Admin/Students/StudentsInformation/FinancialAffairs/FinancialAffairs.jsx";
...
const StudentsInformationPage = () => {
    ...
    return (
        <OriginalTemplatePage>
            ...
            <div
                className='grid grid-cols-6 first:[&>button]:rounded-r-2xl last:[&>button]:rounded-l-2xl mb-8 max-lg:text-sm max-[1150px]:grid-cols-3 max-[500px]:grid-cols-1 max-[1150px]:last:[&>button]:rounded-none max-[1150px]:first:[&>button]:rounded-none max-[1150px]:rounded-2xl overflow-hidden'>
                {page.map((item) => (
                    <button onClick={() => onChange(item.id)} key={item.id}
                            className={`w-full block p-5 text-center border ${item.active ? 'bg-[#FACA15] text-white' : 'bg-white text-[#6B7280]'}`}>{item.title}
                    </button>
                ))}
            </div>
            {showPage()}
